...
# Adapted from twscrape/xclid.py at upstream commit
...
#: A browser-family hint rather than a literal User-Agent. `make_client` turns
#: it into a real UA string and, on the curl backend, into a matching TLS
#: fingerprint. A hard-coded UA string would claim one browser while the
#: handshake shows another, which is a stronger signal than sending no UA hint.
DEFAULT_BROWSER_HINT = "@chrome"


def _make_client(
    proxy: str | None = None,
    cookies: dict[str, str] | None = None,
    user_agent: str = DEFAULT_BROWSER_HINT,
    seed: int | None = None,
) -> HttpClient:
    client = _make_http_client(headers={"user-agent": user_agent}, proxy=proxy, seed=seed)
...
class XClIdGen:
    ...
    async def create(
        ...
        clt = _make_client(proxy=proxy, cookies=cookies, user_agent=user_agent, seed=seed)
        try:
